<?php
	...
	if(ISSET($_POST['add_guest'])){
		...
		$query2 = $conn->query("SELECT * FROM `ctransaction` WHERE `checkin` = '$checkin' && `cabid` = '$_REQUEST[cabid]' && `status` = 'Pending'") or die(mysqli_error());
		$row = $query2->num_rows;
		if($checkin < date("Y-m-d", strtotime('+8 HOURS'))){	
				...
				if($row > 0){
					echo "<div class = 'col-md-4'>
							...
							$q_date = $conn->query("SELECT * FROM `ctransaction` WHERE `status` = 'Pending'") or die(mysqli_error());
						...
						if($guest_id = $fetch['guest_id']){
                            ...
                            $conn->query("INSERT INTO `ctransaction`(guest_id,cabid,cabtype,status,checkin) VALUES('$guest_id','$cabid','$cabtype','Pending', '$checkin')") or die(mysqli_error());
							header("location:reply_cab.php");
						}else{
