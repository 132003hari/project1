	...
	<div class = "container-fluid">
		<div class = "panel panel-default">
			<div class = "panel-body">
				...
				<?php
					$query = $conn->query("SELECT * FROM `ctransaction` NATURAL JOIN `guest`  WHERE `transaction_id` = '$_REQUEST[transaction_id]'") or die(mysqli_error());
					$fetch = $query->fetch_array();
				?>
				<br />
				<form method = "POST" enctype = "multipart/form-data" action = "save_form2.php?transaction_id=<?php echo $fetch['transaction_id']?>">
					...
					<div class = "form-inline" style = "float:left;">
						<label>cabtype</label>
						<br />
						<input type = "text" value = "<?php echo $fetch['cabtype']?>" class = "form-control" size = "20" disabled = "disabled"/>
					</div>
					<div class = "form-inline" style = "float:left; margin-left:20px;">
						<label>cabid</label>
						<br />
						<input type = "number" min = "0" max = "999" name = "cabid" class = "form-control" required = "required"/>
					</div>
					<div class = "form-inline" style = "float:left; margin-left:20px;">
						<label>Days</label>
						<br />
